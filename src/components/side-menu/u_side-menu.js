   ...
   const arr =  Object.entries(menu).map(([key, value]) => {
        ...
        return `
            <li  ${addClass(s['sub-menu-item'], info ? flex_list: '')}>
                ...
                ${info ? 
                    `
                    <span  ${addClass(value[1] ? notification : '', ['Home', 'Profile'].includes(key) ? '' : s['sub-menu-item__value'])}>
                        ${value[0]}
                        </span>
                    ` 
                    : ''}
...
export  const navList = new Map([['', navItem(topMenu)], 
['Explore', navItem(exploreNav)], ['My Books', navItem(myBooksNav)], ['Admin', navItem(adminNav)]
])
